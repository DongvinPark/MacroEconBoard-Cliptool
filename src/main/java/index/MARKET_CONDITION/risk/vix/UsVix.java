package index.MARKET_CONDITION.risk.vix;
...
import index.MARKET_CONDITION.dto.VixDto;
import index.MARKET_CONDITION.dto.VixDtoForJson;
import utils.Logger;
...
public class UsVix implements Index {
  ...
  public boolean convertToJson(String inputCsvPath, String jsonDirPath) {
    try {
      ...
      try( BufferedReader br = Files.newBufferedReader(inputCsv) ) {
        ...
        String line;
        List<VixDto> dtoList = new ArrayList<>();
        while( (line=br.readLine()) != null ){
          String[] tokens = line.split(",");
          if(tokens.length < US_VIX_CSV_HEADER_LENGTH){
            continue;
          }
          ...
          dtoList.add(
              new VixDto(date, getDoubleVal(interestVal))
          );
        }//wh
        ...
        Map<Integer, List<VixDto>> yearMap = new TreeMap<>();
        for (VixDto dto : dtoList) {
        ...
        for( Map.Entry<Integer, List<VixDto>> entry : yearMap.entrySet() ) {
          int year = entry.getKey();
          List<VixDto> yearData = entry.getValue();
          List<VixDtoForJson> yearJsonData = new ArrayList<>();

          for (VixDto dto : yearData) {
            yearJsonData.add(
                new VixDtoForJson(dto.time.toString(), dto.value)
            );
          }

          Path outputFile = outputDir.resolve(
              year + US_VIX_JSON_SUFFIX + JSON_EXTENSION
          );
