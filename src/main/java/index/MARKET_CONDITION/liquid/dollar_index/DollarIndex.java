package index.MARKET_CONDITION.liquid.dollar_index;
...
public class DollarIndex implements Index {
  ...
  public boolean convertToJson(String inputCsvPath, String jsonDirPath) {
    try {
      ...
      try( BufferedReader br = Files.newBufferedReader(inputCsv) ) {
        ...
        String line;
        List<DollarIndexDto> dtoList = new ArrayList<>();
        while( (line=br.readLine()) != null ){
          String[] tokens = line.split(",");
          if(tokens.length < US_DOLLAR_INDEX_CSV_HEADER_LENGTH){
            continue;
          }
          ...
          dtoList.add(
              new DollarIndexDto(date, getDoubleVal(interestVal))
          );
        }//wh
        ...
        Map<Integer, List<DollarIndexDto>> yearMap = new TreeMap<>();
        for (DollarIndexDto dto : dtoList) {
        ...
        for( Map.Entry<Integer, List<DollarIndexDto>> entry : yearMap.entrySet() ) {
          int year = entry.getKey();
          List<DollarIndexDto> yearData = entry.getValue();
          List<DollarIndexDtoForJson> yearJsonData = new ArrayList<>();

          for (DollarIndexDto dto : yearData) {
            yearJsonData.add(
                new DollarIndexDtoForJson(dto.time.toString(), dto.value)
            );
          }

          Path outputFile = outputDir.resolve(
              year + US_DOLLAR_INDEX_JSON_SUFFIX + JSON_EXTENSION
          );
